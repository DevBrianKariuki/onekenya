...
if (isset($_POST['register'])) {
	...
	if (strlen($phone) < 10 or strlen($phone) > 10 and !is_numeric($phone)) {
		header("Location: /ONEKENYA1/register.php?error=Please enter a valid phone number");
	} else if (strlen($password_1) < 6) {
		header("Location: /ONEKENYA1/register.php?error=Password should be more than 6 characters");
	}else if ($password_1 != $password_2) {
		header("Location: /ONEKENYA1/register.php?error=Passwords do not match");
	}else {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			if ($user['email'] === $email) {
				header("Location: /ONEKENYA1/register.php?error=Email is already registered");
				exit();
			}
		}else{
			...
			if (mysqli_query($db, $insert) === true) {
				...
				header("Location: /ONEKENYA1/register.php?error=An unknown error occured");
			}
		}
	}
	...
	header("Location: /ONEKENYA1/register.php");
	exit();
}
